...
**No Lock In** Dat does not import or copy your data into specialized databases. Dat scans and stores filesystem metadata while keeping your data in place. This means that you can easily move your data around and keep it intact in its original form on the filesystem. You can even simultaneously host your data on HTTP along with Dat to ensure backwards compatibility with existing web tools. You'll never be locked into using the Dat software.

**Distributed Storage** Dat is built to better distribute storage and bandwidth costs throughout the network. Data hosting and bandwidth are some of the main costs for long-term data storage. We have plans for public hosting of data, but will prioritize long-term storage through user fees.

**No Centralized Servers** Dat does not currently host any data, except for caching on datproject.org. All data is transferred directly between peers and has little reliance on Dat maintaining servers (we have public servers for peers to help discover each other, but those use very little bandwidth and anyone can run them).
